...
# 第三方库导入
from fontTools.ttLib import TTFont  # 字体处理
...
def load_fonts(*font_paths: str) -> Dict[str, TTFont]:
    """
    加载指定路径的字体文件到内存，返回字体对象字典
    """
    fonts = {}
    for path in font_paths:
        font = TTFont(path)  # 加载字体
        fonts[path] = font  # 存储到字典
    return fonts
...
def has_glyph(font: TTFont, glyph: str) -> bool:
    ...
    检查给定字体是否包含指定字符的字形
    """
    for table in font["cmap"].tables:  # 遍历字符映射表
        if table.cmap.get(ord(glyph)):  # 检查字符是否存在
            return True
    return False
...
def merge_chunks(text: str, fonts: Dict[str, TTFont]) -> List[List[str]]:
    """
    将具有相同字体的连续字符合并为簇，优化字体查找
    """
    chunks = []

    for char in text:
        for font_path, font in fonts.items():
            if has_glyph(font, char):  # 检查字体是否支持该字符
                chunks.append([char, font_path])  # 添加字符和字体路径
                break

    cluster = chunks[:1]  # 初始化簇

    for char, font_path in chunks[1:]:
        if cluster[-1][1] == font_path:  # 如果字体相同
            cluster[-1][0] += char  # 合并字符
        else:
            cluster.append([char, font_path])  # 创建新簇
...
def draw_text_v2(
    ...
    """
    在图像指定坐标绘制文本，使用指定大小、颜色和字体
    """
    y_offset = 0  # Y轴偏移
    sentence = merge_chunks(text, fonts)  # 合并文本块

    for words in sentence:
        xy_ = (xy[0] + y_offset, xy[1] - 6)  # 计算坐标

        font = ImageFont.truetype(words[1], size)  # 创建字体对象
        draw.text(
            xy=xy_,
            text=words[0],
            fill=color,
        ...
        box = font.getbbox(words[0])  # 获取文本边界框
        y_offset += box[2] - box[0]  # 更新偏移量
...
class ImagePattern(Enum):
    ...
    FAKE_STATION = 1       # 虚拟车站
    TEXT = 40.2            # 文本
    STATION = 40           # 车站（圆圈 + 黑体字）
    THUMB_TEXT = 60        # 路线种类图标 + 灰字 -> 路线名
    THUMB_INTEND_TEXT = 80 # 缩进的拇指文本
    GREY_TEXT = 40.1       # 灰色文本
    GREY_INTEND_TEXT = 60.1 # 缩进的灰色文本
